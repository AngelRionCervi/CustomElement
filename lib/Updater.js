...
export default (_this) => {
    return {
        u_children(children) {
            ...
            _this.renderChildrenLoops(childrenLoops);
        ...
        u_class_binds(key) {
            const corBinds = _this.binds
                .get(_G.CLASS_BIND)
                .filter((bind) => bind.keys.map((k) => _H.replaceAll(k, "!", "")).includes(key) || bind.conditions.map((c) => _H.replaceAll(c, "!", "")).includes(key));
            if (corBinds.length > 0) _this.renderClassBinds(corBinds);
        },
        u_all_text_binds() {
