...
export default (state) => {
    ...
    return {
        computeExp(exp) {
            ...
            split.forEach((v, i, a) => {
                if (gloablProps.includes(_H.replaceAll(v, NOT, ""))) {
                    const type = gloablProps.find((e) => e === _H.replaceAll(v, NOT, ""));
                    switch (type) {
                        case FALSE:
                            a[i] = false;
                            break;
                    ...
                    const key = _H.replaceAll(v, NOT, "");
                    keysUsed.push(key);
                    a[i] = _H.resolvePath(state, key);
                } else {
                    ...
                    if (numberRegexp.test(v)) {
                        ...
                        a[i] = replaceAll(v, stringDelimiter, "");
                    }
                }
            });
        ...
        parse(str) {
            ...
            return { res: this.computeBlock(str), keys: keysUsed };
        }
    }
}
