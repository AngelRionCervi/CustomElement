...
const NULL = "null";
const globalProps = [TRUE, FALSE, UNDEFINED, NULL];
const NOT = "!";
const stringDelimiter = "'";
const innerExpBlockRegex = /\(([^()]*)\)/g;
const notRegex = /!+/g;
const comparisonRegexp = new RegExp(`(${EQUAL}|${NOT_EQUAL}|${MORE}|${LESS}|${MORE_OR_EQUAL}|${LESS_OR_EQUAL})`); // DEOSNT WORK FOR MORE_OR_EQUAL AND LESS_OR_EQUAL
    ...
    return {
        computeExp(exp) {
            ...
            for (let u = 0; u < split.length; u++) {
                // first detect and convert state vars
                const nakedExp = _H.replaceAll(split[u], NOT, "");
                let res = split[u];
                if (isVar(nakedExp) && !globalProps.includes(_H.replaceAll(split[u], NOT, ""))) {
                    const varVal = _H.resolvePath(state, nakedExp);
                    console.log(varVal, nakedExp, state)
                    res = (isBoolified(split[u]) ? !varVal : varVal).toString();
                } 
                split[u] = res;
                // second translate strings to js values
                if (globalProps.includes(_H.replaceAll(split[u], NOT, ""))) {
                    const type = globalProps.find((e) => e === _H.replaceAll(split[u], NOT, ""));
                    switch (type) {
                        case FALSE:
                            split[u] = false;
                            break;
