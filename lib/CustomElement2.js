...
export default class CustomElement extends HTMLElement {
    ...
    renderLoops(key = null, elem = null) {
        this.filterBindsByKey(key, elem)
            .get(LOOP_BIND)
            .forEach((loop) => {
                ...
                this.updateBinds(null, null, loopOtherBinds);

                const loopLoopBinds = this.detectLoops(loop.elem);
                console.log(loopLoopBinds);
    ...
    updateBinds(key = null, node = null, settedBinds = null) {
        for (const [type, binds] of settedBinds ? settedBinds : this.filterBindsByKey(key, node)) {
            switch (type) {
                case DATA_BIND: {
                    binds.forEach((bind) => {
