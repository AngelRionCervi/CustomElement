    ...
    detectBinds(node = null) {
        if (!node) {
            ...
            const initDataBinds = this.getUniqBinds(DATA_BIND).reduce((acc, attr) => {
                ...
                return [...acc, { elems, condition, key }];
            }, []);
            ...
            const initTextBinds = this.getUniqTextBinds().reduce((acc, elem) => {
                ...
                return [...acc, { elems, results, keys, text: elem.textContent, key: elem.textContent }];
            }, []);
        ...
        for (const [type, binds] of this.binds) {
            ...
            switch (type) {
                case DATA_BIND: {
                    ...
                    if (node) {
                        if (contains) {
                            corBinds = corBinds.filter((bind) => bind.elems.some((el) => node.contains(el)));
                        } else {
                            corBinds = corBinds.filter((bind) => bind.elems.some((el) => el.isEqualNode(node)));
                        }
                    }
                    break;
                }
                case CLASS_BIND: {
                    corBinds = corBinds.reduce((acc, bind) => {
                        if (
                            bind.boundClasses.every((bound) => bound.key !== key) &&
                            bind.boundClasses.every((bound) => bound.condition !== key)
                        )
                            return acc;
                        return [...acc, bind];
                    }, []);
                    if (node) {
                        if (contains) {
                            ...
                            corBinds = corBinds.filter((bind) => bind.elems.some((el) => el.elem.isEqualNode(node)));
                        }
                    }
                    break;
                }
                case TEXT_BIND: {
                    corBinds = corBinds.reduce((acc, bind) => {
                        if (bind.results.every((bound) => bound.key !== key)) return acc;
                        return [...acc, bind];
                    }, []);
                    if (node) {
                        if (contains) {
...
/* to try : 
not saving the elements when scanning for binds
and rescan all dom tree for corresponding updated binds (with setState)
we can't create binds dynamicaly so no need to rescan the loop source node to add new element to a bind
1: detect the binds
2: apply the binds
3: detect the ifs 
4: render the ifs (innerHTML of detected ifs will still have the generated html from the binds)
5: detect the loops
6: render the loops (innerHTML of detected loops will still have the generated html from the binds, this is copied and only the loop-item is rendered)

Result: doesnt work because in some cases like for classes we have to save the initial classes and for text-binds, the original innerHTML
*/