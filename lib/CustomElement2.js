...
export default class CustomElement extends HTMLElement {
    ...
    // todo : target specified loop to rerender, rerender loop if item changed
    renderLoops(key = null) {
        let totalAdded = 0;
        [...this.selectAll(`[${LOOP_BIND}]`)].forEach((el, i, a) => {
            if (el.getAttribute(KEY_ATTR) === key) el.remove();
        });
        this.filterBindsByKey(key) // maybe i cant do that lol
            .get(LOOP_BIND)
            .forEach((loop, index, loops) => {
                const arrayToLoop = this.resolvePath(this.state, loop.key);
                // modify the position of each loops based on the last loop rendered if their parent is the same
                let diff = arrayToLoop.length - loop.size;
                loop.size = arrayToLoop.length;
                loop.items = arrayToLoop;

                if (loops[index + 1] && loop.parent.isSameNode(loops[index + 1].parent)) {
                    totalAdded += loop.size - diff;
                    loops[index + 1].position += diff;
                    console.log(loops[index + 1], diff, totalAdded)
                }

                arrayToLoop.forEach((item, i) => {
                    const node = this.stringToHtml(loop.elem.outerHTML);
                    node.setAttribute(LOOP_INDEX, i);
                    node.setAttribute(KEY_ATTR, loop.key);
                    loop.parent.insertBefore(node, loop.parent.children[loop.position + i]);
                    const children = this.selectAll(`[${LOOP_ITEM}]`, node);
                    children.forEach((child) => (child.innerHTML = item));
                });

                //if (index === 0) loops[index].elem.remove();
            });
    }
    ...
    detectLoops() {
        const loopBinds = [...this.selectAll(`[${LOOP_BIND}]`)].reduce((acc, elem) => {
        ...
        this.binds.set(LOOP_BIND, loopBinds);
    }
