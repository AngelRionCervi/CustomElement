...
export default class CustomElement extends HTMLElement {
    ...
    renderLoops() {
        this.loops.forEach((el, index, loops) => {
            ...
            this.resolvePath(this, loopTo[1]).forEach((item) => {
    ...
    setState(...args) {
        if (!Array.isArray(args[0])) {
            this.setPath(this, args[0], args[1]);
    ...
    updateNodeData(bind) {
        const val = this.resolvePath(this, bind.key);
        if (bind.condition === true) {
            bind.elem.innerHTML = val;
        } else {
            const cond = this.resolvePath(this, bind.condition);
    ...
    updateNodeClasses(binds) {
        ...
        uniqNodes.forEach((node) => {
            ...
            const generatedClasses = sameElsBinds.reduce((acc, bind) => {
                if (bind.condition === true) {
                    return [...acc, this.resolvePath(this, bind.key)]
                } else {
                    const cond = this.resolvePath(this, bind.condition);
    ...
    setPath(object, path, value, separator = ".") {
        path.trim().split(separator).reduce((o, p, i) => {
