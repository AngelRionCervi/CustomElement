...
export default class CustomElement extends HTMLElement {
    ...
    renderLoops() {
        this.binds.get(LOOP_BIND).forEach((loop, index, loops) => {
            [...this.selectAll(`[${LOOP_BIND}]`)].forEach((el) => {
                el.remove();
            });
            this.resolvePath(this.state, loop.key)
                .reverse()
                .forEach((item, i) => {
                    const node = this.stringToHtml(loop.elem.outerHTML);
                    node.setAttribute(LOOP_INDEX, i);
                    loop.parent.insertBefore(node, loop.parent.children[loop.position]);
