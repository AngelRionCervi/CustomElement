    ...
    return {
            ...
            switch (type) {
                ...
                default: {
                    if (node.tagName !== "STYLE") {
                        const vChild = this.buildVdom(node, vElem);
                        vChild.forEach((vc) => {
        ...
        buildLoopChildren(vElem, key, index, variable, baseNode = null) {
            ...
            Object.entries(objToLoop).forEach(([key, value]) => {
                const vChild = this.buildVdom(node.cloneNode(true), vElem, { value, key, variable, index });
                vChild.forEach((vc) => {
                    vElem.children.push(vc);
                });
            });
        ...
        buildIfChildren(vElem, key) {
            [...key.matchAll(_G.LOOP_VAR_REGEX)].forEach((m) => {
                const r = this.findLoopForIf(vElem, m[0]);
                key = key.replace(m[0], r.cache.key);
            });
