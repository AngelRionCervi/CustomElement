...
export default (_this) => {
    ...
    return {
        ...
        getAttributes(node) {
            const possibleAttrs = [_G.LOOP_BIND, _G.LOOP_ITEM, _G.LOOP_INDEX, _G.IF_BIND, _G.CLASS_BIND];
            const nodeAttrs = [];
            possibleAttrs.forEach((q) => {
                if (node.hasAttribute(q)) {
                    const att = node.getAttribute(q);
                    nodeAttrs.push({ name: q, value: att, keysUsed: _H.getKeysUsed(att.split(":").shift()) });
                }
            });
            return nodeAttrs;
        },
        ...
        createVelem(node, parent, cache) {
            ...
            NodeParser(_this.state).parse(node, attributes);
        ...
        getVChildren(type = null, key = null, blackListedNodes = [], starting = tree) {
            ...
            const exploreChildren = (vNodes) => {
                for (let u = 0; u < vNodes.length; u++) {
                    ...
                    console.log(vNodes[u].attributes)
                    if ((vNodes[u].type === type && !key) || (!type && vNodes[u].updateKeys.includes(key))) {
                        res.push(vNodes[u]);
                        continue;
                    }
                    if (type === "if" && vNodes[u].type === type && key && vNodes[u].updateKeys.includes(key)) {
                        res.push(vNodes[u]);
        ...
        update(key, val) {
            ...
            const vChildByKey = this.getVChildren(null, key);
            console.log(vChildByKey, key) // need to update sole nodes based on key
