...
export default (_this) => {
    ...
    return {
        ...
        buildIfChildren(vElem, key) {
            ...
            const vChildren = this.buildVdom(vElem.node, vElem, { display: condition });
            vChildren.forEach((vc) => {
                vElem.children.push(vc);
            });
        ...
        renderIfs(corVelem = this.getFirstVchild("if")) {
            corVelem.forEach((vElem) => {
                if (vElem.condition) {
                    vElem.children.forEach((child) => { // we dont use baseHTML because it breaks the link between vElem and dom node
