...
export default (_this, symbol) => {
    ...
    return {
        ...
        createVelem(node, parent, cache) {
            ...
            NodeParser(_this, store.get(symbol)).parse(vElem);
            return vElem;
        },
        buildLoopChildren(vElem, key, index, variable) {
            const objToLoop = _H.resolvePath(store.get(symbol), key);
        ...
        buildIfChildren(vElem, key) {
            ...
            const condition = StringParser(store.get(symbol)).parse(key);
        ...
        update(key, val) {
            _H.setPath(store.get(symbol), key, val);
            ...
            vChildrenByKey.forEach((vChild) => {
                NodeParser(_this, store.get(symbol)).parse(vChild);
            });
        },
    };
};
