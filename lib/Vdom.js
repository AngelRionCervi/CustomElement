...
export default (_this) => {
    ...
    return {
        ...
        getFirstChildrenLoopBody(key = null, t = tree) {
            const res = [];
            const vNodes = [...t];
            while (vNodes.length !== 0) {
                const curVnode = vNodes.pop();
                if (curVnode.type === "loop") {
                    if (key && curVnode?.options?.key === key) {
                        res.push(curVnode);
                    } else if (!key) {
                        res.push(curVnode);
                    }
        ...
        renderLoops(corVelem = this.getFirstChildrenLoopBody()) {
